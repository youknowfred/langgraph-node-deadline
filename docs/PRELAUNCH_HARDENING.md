...
| # | Sev | Finding | Fix |
| --- | --- | --- | --- |
| T2.1 | **high** | **`LangGraph TimeoutPolicy` does not exist.** README (`:18,:64,:83`) and both demos cite it as the canonical watchdog. The real per-step watchdog is **`step_timeout`** on the compiled graph; the only public `*Policy` types are `RetryPolicy`/`CachePolicy` (verified against LangGraph 1.1.2 — `import TimeoutPolicy` raises `ImportError`). A reader who searches for `TimeoutPolicy` hits a dead end, undermining a package whose entire pitch is LangGraph expertise. | Replace every "TimeoutPolicy" with "LangGraph's `step_timeout` (the super-step watchdog)"; show real wiring `app = graph.compile(); app.step_timeout = 30.0`; keep demo comments honest ("`asyncio.wait_for` stands in for `step_timeout`, which cancels the node task uncooperatively"). |
